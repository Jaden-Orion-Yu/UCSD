%%Problem 5
%
% Objective: Calculate the approximate integral of f(x) = sin(pi*x) -
% (1/2)* cos(2*pi*x) over the interval [a,b] using different integral approximation methods (left
% end, trapezoid, corrected trapezoid). The error of each approximation is
% measured with the true integration, which is just the actual integral of
% f(x). 
%
% Functions called
%   intLE
...
%
f = @(x) sin(pi*x) - ((0.5)*(cos(2*pi*x))); 
F = @(x) (-sin(2*pi*x)/(pi*4)) - (cos(pi*x)/pi);
%

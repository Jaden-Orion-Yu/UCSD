...
for n = 1:12
    stddev = std(T(:,n));
    mu = mean(T(:,n));
    zeta_2 = (log(1+(stddev/mu)^2));
    lambda(n) = log(mu) - (.5*zeta_2);
    zeta(n) = sqrt(zeta_2);
end
% zeta_2 = (log(1+(stddev./mean).^2));
% lambda = log(mean) - (.5.*zeta_2);
% zeta = sqrt(zeta_2);

%% Contruct model PDF, CDF and 10th percentile
t = (80-45)/168;
x = 45:t:80;                  % range of temperature used in the model
%x = linspace(45,80,169);

for n = 1:12
    model_pdf(n,:) = lognpdf((T(:,n)), lambda(n), zeta(n));
    model_cdf(n,:) = logncdf((T(:,n)), lambda(n), zeta(n));
    fiveth_percentile(n) = logninv(0.05, lambda(n), zeta(n));
    ninetyfiveth_percentile(n) = logninv(.95, lambda(n), zeta(n));
end
...
%% Plot CDF
figure(2); hold on;
pcolor(month, transpose(x), model_cdf'); shading flat;
plot(month, fiveth_percentile, 'w', 'LineWidth', 2);
plot(month, ninetyfiveth_percentile, 'w', 'LineWidth', 2);
xlabel('month'); ylabel('temperature (F)'); axis tight;
title({'Annual cycle of temperature in San Diego',...

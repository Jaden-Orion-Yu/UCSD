...
for j=1:length(f_vec) % loop over each frequency to be tested
    ...
    tic; % start timer
        while toc<waittime % read and dump serial data until wait time is
        reached
        dump=fscanf(s_read);
        end
        while flag==0
    ...
    fs_avg=1/dt_avg; % calculate the average sampling frequency from
    dt_avg
    %% Create plot
    ...
    title(['f_{s,average}=' num2str(round(fs_avg)) ' Hz']); % set title as
    sampling rate
    ...
    title(['FFT of AC signals (f_{in}=' int2str(f_vec(j)) ' Hz)']); % set
    title as sampling rate
    xlim([0 f_max*2]);
    % get current plot axes, set font and line width
    set(gca,'FontSize',22,'LineWidth',2);
    set(gcf, 'units', 'normalized'); % set plot sizing to normalized units
